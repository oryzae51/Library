...
cnt_dir <- "/Users/kimhyoungmin/data/072622_HKB_DPF2mut/quant/cnt"
results_dir <- "/Users/kimhyoungmin/data/072622_HKB_DPF2mut/"
setwd("/Users/kimhyoungmin/Library/Mobile\ Documents/com~apple~CloudDocs/ETL/Experiment\ data/Cowork/060922_PKH_ZWC")
setwd("/Users/kimhyoungmin/data/072622_HKB_DPF2mut/")

##Automate count data.frame 
#Load data directory
src_dir <- c()
src_dir <- c("/Users/kimhyoungmin/data/072622_HKB_DPF2mut/quant/cnt")
...
names(cnts) <- c("MuTnon_1", "MuTnon_2", "MuTDox_1", "MuTDox_2", "WTnon_1", "WTnon_2", "WTDox_1", "WTDox_2") #change col name

#make condition table for downstream processing
condition <- c("MuTnon", "MuTnon", "MuTDox", "MuTDox", "WTnon", "WTnon", "WTDox", "WTDox")
sampleName <- c("MuTnon_1", "MuTnon_2", "MuTDox_1", "MuTDox_2", "WTnon_1", "WTnon_2", "WTDox_1", "WTDox_2")
treatment <- c("non", "non", "Dox", "Dox", "non", "non", "Dox", "Dox")
cellline <- c("Mut", "Mut", "Mut", "Mut", "WT", "WT", "WT", "WT")
sampleTable <- data.frame(treatment, cellline, sampleName, row.names="sampleName")
#sampleTable <- data.frame(condition, sampleName, row.names="sampleName")
sampleTable$treatment <- as.factor(sampleTable$treatment)
sampleTable$cellline <- as.factor(sampleTable$cellline)
#sampleTable$condition <- as.factor(sampleTable$condition)
all(rownames(sampleTable) == colnames(cnts)) #check condition table is correct

##Conduct DESeq2 - 변화폭 확인
library("DESeq2")
library("ggplot2")
dds <- DESeqDataSetFromMatrix(countData = cnts,
                              colData = sampleTable,
                              design = ~ treatment + cellline + treatment:cellline)
dds$treatment <- relevel(dds$treatment, ref = "non")
dds$cellline <- relevel(dds$cellline, ref = "WT")
ddsDox <- DESeq(dds, test="LRT", reduced = ~ treatment + cellline)
resDox <- results(ddsDox)
head(resDox[order(resDox$log2FoldChange), ], 4)
#check dds

##########################
#Normalized counts for a gene with condition-specific changes over times
data <- plotCounts(ddsDox, which.min(resDox$padj), 
                   intgroup = c("treatment", "cellline"), returnData = TRUE)
data <- plotCounts(ddsDox, "ENSG00000118523.6",
                   intgroup = c("treatment", "cellline"), returnData = TRUE)
ggplot(data, aes(x=treatment, y=count, color=cellline, group=cellline)) + 
  geom_point() + stat_smooth(se=FALSE, method="loess") + scale_y_log10()
#############################

###########################
#Wald tests for the log2 fold changes at individual condition can be investigated using the test argument to results
resultsNames(ddsDox)
resvsDox <- results(ddsDox, name="treatmentDox.celllineMut", alpha = 0.05)
summary(resvsDox)
resvsDox <- lfcShrink(ddsDox, coef="treatmentDox.celllineMut", type="apeglm", res=resvsDox)
plotMA(resvsDox, ylim=c(-5, 5))
resvsDox_padj <- subset(resvsDox, padj<0.05)
resvsDox_f2_padj05 <- subset(resvsDox_padj, abs(log2FoldChange) >= 1)
resvsDox_up <- subset(resvsDox_padj, log2FoldChange > 1)
resvsDox_down <- subset(resvsDox_padj, log2FoldChange < 1)
write.csv(as.data.frame(resvsDox), 
          file = paste(results_dir, "resvsDox.csv", sep = ""))
write.csv(as.data.frame(resvsDox_f2_padj05), 
          file = paste(results_dir, "resvsDox_f2_padj05.csv", sep = ""))

#We can further more cluster significant genes by their profiles, 
betas <- coef(ddsDox)
colnames(betas)
library("pheatmap")
mat <- betas[(rownames(betas) %in% rownames(resvsDox_f2_padj05)), -c(1, 2, 3)]
mat_down <- betas[(rownames(betas) %in% rownames(resTP_down)), -c(1, 2, 3, 4)]
mat <- rbind(mat_up, mat_down)
pheatmap(mat, cluster_cols = FALSE, show_rownames = FALSE, breaks=seq(from=-4, to=3, length=101))
...
####################################################################################
##Draw MA plots
res <- results(dds, contrast = c("condition", "WTDox", "WTnon"), alpha = 0.05)
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_WTDox_vs_WTnon", type="apeglm", res=res)
plotMA(resLFC, ylim=c(-5, 5))
png(paste(results_dir, "MAplot_condition_MuTDox_vs_WTDox.png", sep = ""), width = 130, height = 130, units='mm', res = 600)
plotMA(resLFC, ylim=c(-5, 5))
dev.off()
#########################################################################################

##Conduct DESeq2 - WT vs Mut
...
write.csv(as.data.frame(resLFC), 
          file = paste(results_dir, "resLFC_condition_MuTDox_vs_WTDox.csv", sep = ""))
#padj<0.05, foldchange>2
write.csv(as.data.frame(res_f2_padj05), 
          file = paste(results_dir, "resLFC_f2_padj05_condition_MuTDox_vs_WTDox.csv", sep = ""))
...
p53_kegg <- c("CDKN1A", "GADD45A", "CD82", "GADD45B", "SERPINE1", "PMAIP1", "RRM2", "SESN2", "CCNE2")
png("/Users/hmkim/data/buffer/PolII/Volcanoplot_PolII.png", 
    width = 260, 
    height = 260, 
    units='mm', 
    res = 300)
EnhancedVolcano(resvsDox,
                lab = resvsDox$gene_sym,
                selectLab = p53_kegg,
                x = 'log2FoldChange',
                y = 'padj', 
                pCutoff = 0.05, 
...
pheatmap(sampleDistMatrix,
         ...
         col=colors)
plotPCA(rld, intgroup=c("treatment", "cellline"))
