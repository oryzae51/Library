...
setwd("/Users/hmkim/data/quant_data/073021_EP400NL/cnt")

##Automate count data.frame 
#Load data directory
src_dir <- c()
src_dir <- c("/Users/hmkim/data/quant_data/073021_EP400NL/cnt")
...
names(cnts) <- c("non_1", "non_2", "tet_1", "tet_2") #change col name

#make condition table for downstream processing
condition <- c("non", "non", "tet", "tet")
sampleName <- c("non_1", "non_2", "tet_1", "tet_2")
...
dds <- DESeqDataSetFromMatrix(countData = cnts,
                              colData = sampleTable,
                              design = ~ condition)
dds$condition <- relevel(dds$condition, ref = "non")#Make WT sample level as reference
dds <- DESeq(dds)
...
##Draw MA plots
#MLL1-KO
res_tet <- results(dds, contrast = c("condition", "tet", "non"), alpha = 0.05)
resultsNames(dds)
resLFC_tet <- lfcShrink(dds, coef="condition_tet_vs_non", type="apeglm", res=res_tet)
plotMA(resLFC_tet, ylim=c(-5, 5))
png("/Users/hmkim/data/buffer/EP400NL/MAplot_MLL1.png", width = 130, height = 130, units='mm', res = 600)
plotMA(resLFC_tet, ylim=c(-5, 5))
dev.off()
#########################################################################################
...
ensembl_id <- c(row.names(resLFC_tet))
ensembl_id <- substr(ensembl_id, 1, 15)
restet_genesym <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"),
                         filters = "ensembl_gene_id", 
                         values = ensembl_id, 
                         mart = ensembl)
#remove duplicate
restet_genesym <- restet_genesym[-which(duplicated(restet_genesym$ensembl_gene_id)),]
#add empty values to not assigned id in restet_genesym
notassigned <- setdiff(ensembl_id, restet_genesym$ensembl_gene_id)
...
restet_genesym <- rbind(restet_genesym, notassigned_df)
#add genesym column to res
resLFC_tet$ensembl_gene_id <- c(ensembl_id)
resLFC_tet <- resLFC_tet[c(order(resLFC_tet$ensembl_gene_id)), ]
restet_genesym <- restet_genesym[c(order(restet_genesym$ensembl_gene_id)), ]
resLFC_tet$gene_sym <- restet_genesym$hgnc_symbol
resLFC_tet <- resLFC_tet[!(resLFC_tet$ensembl_gene_id %in% notassigned), ]

#res Cutoff
summary(resLFC_tet)
restet_padj <- subset(resLFC_tet, padj<0.05)
summary(restet_padj)
restet_f2_1.5_padj05 <- subset(restet_padj, abs(log2FoldChange) > log2(1.5))
summary(restet_f2_1.5_padj05)

##Save deg_data
#Raw deg
write.csv(as.data.frame(resLFC_tet), 
          file = "/Users/hmkim/data/buffer/EP400NL/restet_MLL2.csv")
#padj<0.05, foldchange>2
write.csv(as.data.frame(restet_f2_padj05), 
          file = "/Users/hmkim/data/buffer/EP400NL/restet_f2_padj05.csv")
#padj<0.05, foldchange>log2(1.5)
write.csv(as.data.frame(restet_f2_1.5_padj05), 
...
res_df <- data.frame(resLFC_tet)
res_df_rm_EP400P1 <- res_df
res_df_rm_EP400P1 <- subset(res_df_rm_EP400P1, gene_sym!="EP400P1")

GLUT <- c("SLC2A1", "SLC2A2", "SLC2A3", "SLC2A4", "SLC2A10")
png("/Users/hmkim/data/buffer/Volcanoplot_EP400_lowRes_w_EP400NL.png", 
    width = 260/2, 
    height = 260/2, 
    units='mm', 
    res = 300)
EnhancedVolcano(res_df,
                ...
                boxedLabels = FALSE, 
                legendLabSize = 8,
                legendIconSize = 3.0/2,
                drawConnectors = FALSE,
                gridlines.major = FALSE, 
                gridlines.minor = FALSE
)
png("/Users/hmkim/data/buffer/Volcanoplot_EP400_lowRes.png", 
    width = 260/2, 
    height = 260/2, 
    units='mm', 
    res = 300)
EnhancedVolcano(res_df_rm_EP400P1,
                lab = res_df_rm_EP400P1$gene_sym,
                selectLab = "",
                x = 'log2FoldChange',
                y = 'padj', 
                pCutoff = 0.05, 
