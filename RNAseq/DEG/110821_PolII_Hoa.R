...
setwd("/Users/hmkim/data/quant_data/110821_PolII_HOA_quant/cnt")

##Automate count data.frame 
#Load data directory
src_dir <- c()
src_dir <- c("/Users/hmkim/data/quant_data/110821_PolII_HOA_quant/cnt")
...
names(cnts) <- c("polKO_1", "polKO_2", "wt_1", "wt_2") #change col name

#make condition table for downstream processing
condition <- c("polKO", "polKO", "wt", "wt")
sampleName <- c("polKO_1", "polKO_2", "wt_1", "wt_2")
...
####################################################################################
##Draw MA plots
res <- results(dds, contrast = c("condition", "polKO", "wt"), alpha = 0.05)
resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_polKO_vs_wt", type="apeglm", res=res_tet)
...
write.csv(as.data.frame(resLFC), 
          file = "/Users/hmkim/data/buffer/PolII/resLFC.csv")
